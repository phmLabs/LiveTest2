...
<head>
    ...
    <style>
        ...
        #compactToggleButton, .colorLegend {
            text-align: center;
        }
        ...
        .result_failed, .result_error, .result_success, .url_column a {
            color: #fff;
        }

        .result_none, #compactToggleButton {
            background-color: #ccc;
        }

        .standard .testLabelTitleVertical, .compact .testLabelTitleExtended {
            display: none;
        }

        .testLabelTitleVertical {
            overflow: hidden;
            width: 20px;
        }
        ...
        .url_column, .result_column {
            padding: 5px;
        }

        .url_error, .result_error {
            background-color: #3038f2;
        }

        .url_failed, .result_failed {
            background-color: #f24028;
        }

        .url_success, .result_success {
...
<div id="wrapper" class="standard">
    <table>
        ...
        <tr>
            <td></td>
            <td colspan="3">Duration:
                <b><?php echo \Base\Date\Duration::format(floor($information->getDuration() / 1000), '%d day(s) ', '%d hour(s), ', '%d minute(s) ', '%d second(s)'); ?></b>
            </td>
        </tr>
        <tr>
            <td></td>
                ...
                <td colspan="3">
                    <ul>
                        <?php foreach ($connectionStatuses as $status): ?>
                            <li>
                                <a href="<?php echo htmlspecialchars($status->getRequest()->getUri()); ?>"><?php echo htmlentities($status->getRequest()->getUri()); ?></a>
                            </li>
                        <?php endforeach; ?>
                    </ul>
                </td>
    ...
    <table>
        ...
        <tr>
            ...
            <?php foreach ($tests as $test): ?>
                <th class="test_label">
                    ...
                    <span class="testLabelTitleExtended"><?php echo $test->getName(); ?>
                        <br/><?php echo $test->getClassName() ?></span>
                </th>
            <?php endforeach; ?>
        </tr>
        </thead>
        ...
        <?php foreach ($matrix as $url => $testInfo): $testList = $testInfo['tests']; ?>
            <tr>
                <td class="url_column <?php echo getRowClass($testInfo['status']); ?>"><a
                        href="<?php echo htmlspecialchars($url) ?>"
                        target="_blank"><?php echo htmlentities($url); ?></a></td>
                <?php
